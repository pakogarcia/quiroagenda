...
const generateWhatsappReminderPrompt = ai.definePrompt({
  ...
  '¡Hola {{clientName}}! 👋\n\nTe escribimos de parte de {{#if businessName}}_{{businessName}}_{{else}}nuestro centro{{/if}} para recordarte tu cita del *{{appointmentDateTime}}*.\n\nPor favor, si no puedes acudir, avísanos con la mayor antelación posible.{{#if customMessage}}\n\n{{customMessage}}{{/if}}\n\n¡Te esperamos! ✨{{#if website}}\n\nWeb: {{website}}{{/if}}{{#if instagram}}\nInstagram: {{instagram}}{{/if}}{{#if facebook}}\nFacebook: {{facebook}}{{/if}}{{#if tiktok}}\nTikTok: {{tiktok}}{{/if}}{{#if youtube}}\nYouTube: {{youtube}}{{/if}}'
  `,
});
